...
void mdelay(tU32 delayInMs)
{
  ...
  T0TCR = 0x02;                            // 0x02 = 0b 10 -> Timer counter and Prescle counter are synchronously reset.
  T0PR  = (PERIPHERAL_CLOCK / 1000) - 1;   // Set prescaler to zero.
  ...
  T0IR  = 0xFF;          // Reset all interrrupt flags in interrupt register of Timer 1.
  T0MCR = 0x04;          // Stop timer on match, effectively disabling Timer Counter and Prescale Counter (setting 0 to T1TCR[0])
  T0TCR = 0x01;          // 0x01 = 0b 1 -> Timer counter and Prescale counter are enabled to count.
  ...
  T0TCR = 0x00;         // Resetting entire TCR register of Timer 1.
}
